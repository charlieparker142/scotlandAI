...
@ManagedAI("MrXAI")
public class MrXAI implements PlayerFactory, MoveVisitor{

    public Integer score;
    public Integer destination;
    public Integer count;
    public Integer index;
    public List<Integer> placesAlreadyBeen;
    ...
    public MrXAI() {
        ...
        this.count = 0;
        this.index = 0;
        this.placesAlreadyBeen = new ArrayList<>();
    }
	...
	private class MyPlayer implements Player {
		...
		public void makeMove(ScotlandYardView view, int location, Set<Move> moves,
            ...
            if(view.getCurrentPlayer() == Colour.BLACK) {
                ...
                int highestscore = highestScoreMoveMrX(view, location, moves);
                ...
                System.out.println("MrX chooses " + bestmoves.get(randomInt));
                callback.accept(bestmoves.get(randomInt));   
                Move move2 = bestmoves.get(randomInt);
                Integer lastDestination = findTheDestination(move2);
                System.out.println(lastDestination);
                addToLastDestinations(lastDestination);
    ...
    public Integer highestScoreMoveMrX(ScotlandYardView view, int location, Set<Move> moves){
        //this.moves.clear();
    ...
    public Integer scoreOfMove(Move move, ScotlandYardView view, int location) {
        ...
        move.visit(this);
        Integer a = howManyMovesToDetective(move, view, location, this.destination);
        Integer b = varietyOfRoutes(move, view, location, this.destination);
        if (this.placesAlreadyBeen.contains(this.destination)) b = b - 5;
        return 2*a + b;
    }

    public Integer howManyMovesToDetective(Move move, ScotlandYardView view, int location, int destination) {
        // for the edges from the location...
        boolean detectiveHere = false;
        Collection<Edge<Integer, Transport>> edges =  view.getGraph().getEdgesFrom(view.getGraph().getNode(destination));

        while (detectiveHere == false) {

            // checking if any detectives are 1 moves away from potential move
		    for (Edge<Integer, Transport> edge : edges) {
                int destinationOfEdge = edge.destination().value();
                if (detectiveisHere(destinationOfEdge, view)) {
                    this.count = 1;
                    detectiveHere = true;
                } 
            }
    ...
    public boolean detectiveisHere(int destination, ScotlandYardView view) {
    ...
    public void addToLastDestinations(Integer destination) {
        this.placesAlreadyBeen.add(destination);
    }
}
